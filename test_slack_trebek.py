...
class TestSlackTrebek(unittest.TestCase):
    def setUp(self):
        d = self.get_setup_json()
        self.room_message = entities.TrebekMessage(d)
    ...
    def get_setup_json(self):
        with open('test-slack.json') as data:
            d = json.load(data)
        return d

    def create_bot_with_dictionary(self, room_dictionary):
        msg = entities.TrebekMessage(room_dictionary)
    ...
    def test_given_json_dictionary_slack_object_is_parsed(self):
        with open ('test-slack.json') as data:
            d = json.load(data)
        t = entities.TrebekMessage(**d)
        t.assign_from_slack()
        self.assertEqual(t.message, "jeopardy") 
        self.assertEqual(t.user_name, "James A")
    ...
    def test_when_get_response_message_is_called_user_name_is_saved(self):
        ...
        key = trebek.Trebek.hipchat_user_key.format('U2147483697')
        self.assertTrue(self.trebek_bot.redis.exists(key))

        user_name = self.trebek_bot.redis.get(trebek.Trebek.hipchat_user_key.format('U2147483697')).decode()
    ...
    def test_number_is_formatted_as_currency(self):
        ...
        currency = self.trebek_bot.format_currency("-100")
        self.assertEqual("_-$100_", currency)

        currency = self.trebek_bot.format_currency("-1000000000")
        self.assertEqual("_-$1,000,000,000_", currency)
    ...
    def test_user_leaderboard_value_returned(self):
        ...
        expected = "Leaderboard for {0} {1}:\n\n".format(dt.strftime("%B"), dt.year)
    ...
    def test_jeopardy_round_can_start_from_nothing(self):
        ...
        expected += "*\"CAVEAT EMPTOR.  LET THE BUYER BEWARE\"* (Air Date: 18-Oct-2001)"
    ...
    def test_given_incorrect_answer_user_score_decreased(self):
        # Arrange 
        d = self.get_setup_json()
        d['text'] = 'some test answer'
        bot = self.create_bot_with_dictionary(d)
        bot.redis = fakeredis.FakeStrictRedis()
        bot.get_question()
        response = bot.get_response_message()
        user_score_key = "{0}:{1}".format(bot.user_score_prefix,
                self.trebek_bot.room_message.user_id)
    ...
    def test_given_correct_answer_user_score_increased(self):
        # Arrange 
        d = self.get_setup_json()
        d['text'] = "what is Let's Make a deal"
        bot = self.create_bot_with_dictionary(d)
        bot.redis = fakeredis.FakeStrictRedis()
        bot.get_question()
        response = bot.get_response_message()
    ...
    def test_given_incorrect_answer_time_is_up_response(self):
        ...
        self.assertEqual(response, "Time is up! The correct answer was: *Let's Make a Deal*")
    ...
    def test_user_lifetime_loserboard_value_includes_multiple_months(self):
        d = self.get_setup_json()
        d['text'] = "show me the lifetime loserboard"
        bot = self.create_bot_with_dictionary(d)
        self.create_user_scores(bot)
        response = bot.get_response_message()

        expected =  "1. Legacy Score: $5\n"
        expected += "2. Allen: $40\n"
        expected += "3. Mark: $60\n"
        expected += "4. Melvin: $100\n"
        expected += "5. Cordarrell: $140\n"
        self.assertEqual(expected, response)

    def test_user_lifetime_leaderboard_value_returned(self):
        d = self.get_setup_json()
        d['text'] = "lifetime leaderboard"
        bot = self.create_bot_with_dictionary(d)
        self.create_user_scores(bot)
        response = bot.get_response_message()

        expected =  "1. Arian: $10,860\n"
        expected += "2. Darren S: $1,000\n"
        expected += "3. Zach: $824\n"
        expected += "4. Alex: $450\n"
        expected += "5. Richard: $400\n"
        self.assertEqual(expected, response)
